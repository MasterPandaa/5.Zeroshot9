...
class Board:
    ...
    def is_square_attacked_by(self, r: int, c: int, attacker_color: str) -> bool:
        ...
        # Pawns
        dir = (
            -1 if attacker_color == WHITE else 1
        )  # pawns attack forward relative to their color
        for dc in (-1, 1):
            rr, cc = r + dir, c + dc
            if self.inside(rr, cc):
                p = self.board[rr][cc]
    ...
    def is_legal_move(
        self, move: Tuple[Tuple[int, int], Tuple[int, int]], color: str
    ) -> bool:
        (r1, c1), (r2, c2) = move
        p = self.board[r1][c1]
...
class ChessUI:
    ...
    def handle_mouse_click(self, pos: Tuple[int, int]):
        ...
        if self.selected:
            ...
            if any((r, c) == t for t in self.legal_targets):
                ...
                if (
                    p
                    and p.color == self.board.to_move == WHITE
                    and self.board.to_move == WHITE
                ):
    ...
    def draw(self):
        ...
        if self.selected:
            ...
            pygame.draw.rect(
                self.screen, SELECTED_OUTLINE, (sx, sy, TILE_SIZE, TILE_SIZE), 4
            )
            ...
            for tr, tc in self.legal_targets:
                ...
                if target_piece is None:
                    ...
                    pygame.draw.rect(
                        self.screen,
                        (230, 90, 90),
                        (tx + 6, ty + 6, TILE_SIZE - 12, TILE_SIZE - 12),
                        3,
                    )
        # Draw pieces
        for r in range(8):
    ...
    def draw_piece(self, piece: Piece, r: int, c: int):
        ...
        try:
            text = self.font.render(
                sym if sym else "",
                True,
                (0, 0, 0) if piece.color == BLACK else (255, 255, 255),
            )
            # Shadow for contrast
            ...
            pygame.draw.circle(
                self.screen, color, (x + TILE_SIZE // 2, y + TILE_SIZE // 2), radius
            )
            # mark kind with small letter
            lbl = self.small_font.render(
                piece.kind, True, (0, 0, 0) if piece.color == WHITE else (255, 255, 255)
            )
            self.screen.blit(
                lbl,
                (
                    x + TILE_SIZE // 2 - lbl.get_width() // 2,
